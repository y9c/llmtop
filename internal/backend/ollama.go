...
func (Ollama) Detect(body string) bool { return strings.Contains(body, `"models"`) && strings.Contains(body, `"size_vram"`) }
...
func (Ollama) Parse(body string) (metrics.Snapshot, error) {
	...
	s.GPUMemTotalMB = s.GPUUsedMB * 2 // rough estimate: Ollama loads model in memory
